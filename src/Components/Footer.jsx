...
import { AppName, DesignedBy, footerItems, footerLinks, footerTitle } from '../Constants';

const Footer = () => {
  return (
    <div className="flex flex-col pt-[10%] lg:pt-[5%] font-lato bg-gradient-to-b from-indigo-100 dark:from-[#222226]" id="contact_us ">
      <div className="flex flex-col lg:flex-row md:flex-row ">
        <div className="mx-[2%] lg:w-[50%] md:w-[40%]">
          <div className="flex flex-row">
            <img src={sample_logo} alt="sample_logo" className="h-10 mx-3 my-2" />
            <h1 className="font-bold font-[lato] text-[24px] my-2">{AppName}</h1>
          </div>
              ...
              return (
                <div className='flex flex-col mx-5'>
                  ...
                  <div className='flex flex-col mt-7'>
                    { value.links.map((valueElement)=>{
                      return (
                        <a href={valueElement.link} className='my-1'>{valueElement.text}</a>
                      )
                    })}
                  </div>
                </div>
        ...
        <div className="flex flex-col md:flex-row mt-10 md:mt-0 lg:mt-0 ">
          {
            footerLinks.map((value)=>{
              return <a href={value.link} className={`text-sm font-[lato] font-bold mx-2 mt-2 text-blue-600`}>{value.text}</a>
            })
          }
        </div>
